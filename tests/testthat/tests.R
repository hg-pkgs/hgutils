context("survEstimateTime - NO NEWDATA")
suppressWarnings(library(rms))
library(HGUtils)

test_that("Same estimates in survEstimateTime for survest and survfit with no newdata", {
...
context("survEstimateTime - NEWDATA")
test_that("Same estimates in survEstimateTime for survest and survfit with newdata", {
    fit1 = cph(Surv(time = time, event = status == 2) ~ age + sex, data = lung, x = TRUE, y = TRUE, surv = TRUE)
    fit2 = cph(Surv(time = time, event = status == 2) ~ age + sex, data = lung, x = FALSE, y = FALSE, surv = TRUE)
    ...
    for (i in 1:100) {
        ...
        expect_identical(survEstimateTime(fit1, surv, newdata)$time, survEstimateTime(fit2, surv, newdata)$time)
    }
})
...
test_that("Same estimates in survEstimateTime for survest and survfit with newdata", {
  for (i in 1:100) {
    ...
    if (is.unsorted(y0)) expect_error(seperate_values(y0, distance=space))

    y0 = sort(y0)
    res = seperate_values(y0, space)
    expect_equal(res>=0 && res<=1, TRUE)

    expect_equal(all(sapply(1:length(res), function(x) abs(res[x]-res[-x])) >= space-0.0001), TRUE)
